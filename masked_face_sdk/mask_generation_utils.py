...
def warp_mask(
    ...
    print('running warp_mask functions ....')
    ...
    fake_image = np.zeros((dst_image.shape[0], dst_image.shape[1]), dtype=np.uint8)
    print('shape of fake image ', fake_image.shape)
    mask_area = cv2.fillConvexPoly(
        np.zeros((dst_image.shape[0], dst_image.shape[1]), dtype=np.uint8),
        dst_landmarks, (255,255,255)
    )
    result = dst_image.copy()
    result[mask_area > 0] = 233
...
def end2end_mask_generation(
    ...
    print("sampling mask data ", sampling_mask_data)
    face_with_mask = apply_mask_to_image_with_face(
        image,
        sampling_mask_data,
        face_landmarks
    )
    print("face with mask ", face_with_mask)
